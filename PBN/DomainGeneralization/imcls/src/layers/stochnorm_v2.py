...
__all__ = ['StochNorm1d_v2', 'StochNorm2d_v2', 'convert_model']


class _StochNorm_v2(nn.Module):

    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True, p=0.5):
        super(_StochNorm_v2, self).__init__()
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            in_var, in_mean = torch.var_mean(input, dim=(2, 3), keepdim=False, unbiased=False)
            z_2 = (input - in_mean.view(*shape)) / (in_var.view(*shape) + self.eps).sqrt()
            z_2 = z_2 * self.weight.view(*normalized_shape) + self.bias.view(*normalized_shape)

            l = torch.from_numpy(
                    np.random.binomial(n=1, p=0.2, size=self.num_features).reshape(1, self.num_features, 1,
                                                                                      1)).float().cuda()
            z_1 = (1 - l) * z_1 + l * z_2
...
class StochNorm1d_v2(_StochNorm_v2):
...
class StochNorm2d_v2(_StochNorm_v2):
    r"""
...
class StochNorm3d_v2(_StochNorm_v2):
    r"""
